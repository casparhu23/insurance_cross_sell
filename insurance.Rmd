...
```{r}
riMod <- lmer(Annual_Premium ~ Age+Gender +Driving_License+Vehicle_Insured+Vehicle_Age+Vehicle_Damage+(1|Region_Code), 
              data = train)
...
##Tuning the randomforest model using for each
```{r}
library(randomForest)
library(caret)
...
##Tuning the randomforest model with traditional method
...
boost_pred_class <- rep(0, length(boost_preds_1))  #create one column of 1 
boost_pred_class[boost_preds_1 >= oc$MaxSpSe$Global$optimal.cutoff$cutoff[1]] <- 1
# replace those larger than cut off point as 0
...
When we choose the optimal cut point instead of 0.5, we got accuracy is 76% and sensitivity is 69.38%
...
roc_1 = roc(test_subset$new_response, boost_preds_1) # naive model
# Calculate final model ROC
roc_2 = roc(test_subset$new_response, boost_preds) # final model
