...
The higher mean decrease in Gini, the higher variable importance for that variable would be. Therefore, we can conclude that whether vehicle damaged or not and whether customers already has auto insurance or does not have auto insurance are the most important variables. Next step, we will use XGBoost to build model and see if XGBoost model can perform better than random forest model.
